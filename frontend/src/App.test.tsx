...
describe("player pairing setup", () => {
  ...
  it("keeps the board grid and cells square on wide layouts", async () => {
    const fetchMock = vi
      .fn()
      .mockResolvedValueOnce({ ok: true, json: async () => unavailableCapabilitiesResponse })
      .mockResolvedValueOnce({ ok: true, json: async () => initialResponse });
    ...
    expect(board).toHaveClass("aspect-square", "grid-cols-3", "grid-rows-3", "mx-auto");

    const cells = within(board).getAllByRole("gridcell");
    expect(cells).toHaveLength(9);
    for (const cell of cells) {
      expect(cell).toHaveClass("min-h-0", "min-w-0");
      expect(within(cell).getByRole("button")).toHaveClass("aspect-square", "min-h-0");
    }
  });
