    ...
    <main className="min-h-screen overflow-hidden bg-[#0b0d12] text-[#f7f7f2]">
      ...
      <div className="relative mx-auto flex min-h-screen w-full max-w-7xl flex-col px-5 py-6 sm:px-8 lg:px-12">
          ...
          <div className="max-w-xl">
            ...
            <div className="mt-10 rounded-3xl border border-white/10 bg-white/[0.025] p-5 backdrop-blur-sm sm:p-6">
              ...
              <Button aria-label={isGameStarted ? "Game Started" : "Start game"} className="mt-5 h-11 w-full rounded-xl bg-[#d8ff56] font-semibold text-[#101219] hover:bg-[#e5ff91] disabled:bg-white/10 disabled:text-white/40" disabled={isLoading || isGameStarted} onClick={handleStartGame}>
                {isGameStarted ? "Game Started" : isLoading ? "Startingâ€¦" : "Start game"}
              </Button>
            </div>
          </div>
          ...
          <MagicCard className="rounded-[2rem] bg-[#11141c] shadow-2xl shadow-black/30" gradientColor="#d8ff56" gradientFrom="#d8ff56" gradientTo="#ff8a65" gradientOpacity={0.2}>
            <div className="p-5 sm:p-7">
              <div className="mb-6 flex items-start justify-between gap-4">
                <div>
                  <p className="text-xs font-semibold uppercase tracking-[0.2em] text-white/35">Round 01 Â· {pairingLabel(gameState.players)}</p>
              ...
              <div aria-live="polite" className="mb-5 flex items-center justify-between rounded-2xl border border-[#d8ff56]/15 bg-[#d8ff56]/[0.06] px-4 py-3">
                <div>
                  <p className="text-xs uppercase tracking-[0.18em] text-[#d8ff56]/70">Current turn</p>
                  <p className="mt-1 text-sm font-medium text-white">{gameStatus} <span className="font-normal text-white/45">Â· server state</span></p>
                </div>
                <span className="flex h-9 w-9 items-center justify-center rounded-xl bg-[#d8ff56] text-lg font-semibold text-[#101219]">{gameState.currentPlayer ?? gameState.winner ?? "â€”"}</span>
              </div>
              ...
              <div className="mt-5 flex items-start gap-3 text-sm text-white/45">
                ...
                <p>{!isGameStarted ? "Choose a controller for each player, then start the game." : isLoading ? "Syncing with the game serverâ€¦" : currentController === "ai-agent" ? `Waiting for the AI Agent to propose a ${gameState.currentPlayer} move through MCP.` : currentController === "cpu" ? "The CPU is thinkingâ€¦" : gameState.status === "in_progress" ? "Moves are validated and applied by FastAPI." : "The server has closed this board. Reset to play again."}</p>
              </div>
        ...
        <footer className="flex flex-col gap-2 border-t border-white/10 pt-5 text-xs text-white/30 sm:flex-row sm:items-center sm:justify-between">
          ...
          <span>3 Ã— 3 Â· X starts Â· no second rules engine</span>
        </footer>
      </div>
    </main>
  );
