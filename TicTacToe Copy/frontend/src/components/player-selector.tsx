...
export function PlayerSelector({ player, value, onChange, aiAgentAvailable = false, disabled: selectionDisabled = false }: PlayerSelectorProps) {
  ...
  const selectedLabel = playerOptions.find(({ id }) => id === value)?.label ?? "Human";

  return (
    <fieldset aria-label={`Player ${mark}`} className="min-w-0 space-y-3">
      <legend className="hidden items-center justify-start gap-2 text-left text-sm font-medium text-white/70 sm:flex">
      ...
      <details open className="group">
        <summary className="flex cursor-pointer list-none items-center justify-center gap-3 rounded-2xl border border-white/10 bg-white/[0.03] px-3 py-3 text-center text-sm font-medium text-white/70 sm:hidden [&::-webkit-details-marker]:hidden">
          <span className="flex items-center gap-2">
            <span className="flex h-7 w-7 items-center justify-center rounded-lg bg-white/10 text-xs font-semibold text-[#d8ff56]">{mark}</span>
            Player {mark}
          </span>
